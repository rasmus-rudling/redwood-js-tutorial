...
describe('contacts', () => {
  ...
  scenario('creates a contact', async () => {
    const result = await createContact({
      input: {
        name: 'Rasmus',
        email: 'rasmusrudling@gmail.com',
        message: 'Hallå där!',
      },
    })

    expect(result.name).toEqual('Rasmus')
    expect(result.email).toEqual('rasmusrudling@gmail.com')
    expect(result.message).toEqual('Hallå där!')
  })
})
